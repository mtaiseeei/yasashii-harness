...
- 全スプリント共通の重要事項は `docs/spec/*.md` に置く。過去スプリント固有の判断は `docs/sprints/sprint-*.md` に閉じ込める。
- 進捗は Generator だけが `docs/progress/sprint-*.md` に書く。Evaluator は読み取り専用。
...
- 実行順はファイル名ソートに依存せず、必ず `docs/sprints/current.md` に `Current ID`、`Contract`、
  `Progress`、`Feedback`、`Next Planned` を明記する。
...
- 合格済み Sprint への追加修正、または現在 Sprint の受け入れ基準に含まれない軽微なUI調整・文言整理・導線修正・
  小さな技術負債返済は、ユーザーが「Patch 1」と言わなくても Planner に Patch Sprint 契約を作らせる。
- 例: ユーザーが「Sprint 5 のUIを少し直したい」と言った場合、既存の
  `docs/sprints/sprint-005-patch-*.md` を確認し、次の空きID（例: `sprint-005-patch-001`）を自動採番する。
...
- 完了後、対象の `docs/progress/sprint-*.md` に自己評価と引き渡し事項（起動方法・URL・テストシナリオ）が
  書かれていることを確認する。
...
4. **フィードバックを最優先で処理する** — Generator は新スプリント着手前に、前スプリントの
   不合格フィードバックを修正する。
5. **起動手順を必ず記載する** — Generator は対象の `docs/progress/sprint-*.md` に起動コマンドを毎回明記し、
...
- Planner: 「次のアイデアについて、まずユーザーが決めるべき重要判断を最大3つの選択式質問にして。
  ...
  `docs/spec/*.md`、`docs/sprints/current.md`、初回の `docs/sprints/sprint-001.md` に展開して：
  『<ユーザーのプロンプト>』」
...
- Evaluator: 「Current ID のスプリントを利用可能なブラウザ検証面で検証し、対応する
  `docs/feedback/sprint-*.md` に合否を書いて」
