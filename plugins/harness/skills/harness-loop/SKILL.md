...
通常どおりresolverへ渡し、desired tierが同じ同一Sprint retryならresumeしてよい。これは今後Harnessを
...
- `--current-model-tier`には、resolver呼出し前にstate.mdから読んだ現在の`Model Tier`を必ず渡す。
  ...
  resolverが返すdesired tierと現在tierが異なる場合だけGeneratorをfreshにする。`strong`から`strong`の継続は、
  high-risk、Evaluator推薦、availability fallback、retry閾値のどの理由でも同一Sprintならresumeしてよい。
...
- 実効設定とwarningをdispatch前に確認する。`dispatch-ready` はhostへ渡す面と利用可能値を確認した状態であり、
  実際にそのmodel / effortで起動した証明ではない。`launch-verified` はhost側session metadataまたはtraceを
  取得できた場合だけ使う。metadataが無ければ実起動は`unverified`と記録する。
...
- `balanced`（既定）: 同じroleのAgentをhostが許す範囲でSprint間resumeする。GeneratorとEvaluatorは
  常に別Agent / 別作業単位で、相互のsessionをresumeしない。
- `fresh`: 新Sprint境界でGeneratorとEvaluatorをそれぞれfreshにする。同一Sprintのretryは
  Generatorをresumeし、EvaluatorもGeneratorとは別のまま同一Sprintの評価文脈をresumeしてよい。
- 例外として、Model Tierが`standard`から`strong`、または`strong`から`standard`へ変わるときは
  `balanced`でもGeneratorをfreshにする。同じtierを継続するだけならfresh化しない。
...
- resolverのdesired tierとcurrentModelTierを比較し、異なる場合はstate.mdの`Model Tier`をdesired tier、
  ...
  `balanced`かつhostがresume可能なら既存Generatorをresumeする。
...
- **合格** → Status を `done`、Retry Countを0にし、`Current ID`を次のスプリントへ進める。次Sprintがある場合は、
  `Model Tier`を最後に実dispatchした値のまま保持し、`Rotate: none`としてStep 2へ進む。Step 2がそのtierを
  `currentModelTier`としてresolverへ渡し、desired tierとの比較後にstateを更新してからdispatchする。
...
- **不合格（implementation-issue）** → Retry Count を +1してstate.mdへ記録する。1回目はstandardの
  Generatorをresumeしてよい。2回目はModel Tierを`strong`、Rotateを`model-escalation`へ更新してから、
  古いLuna GeneratorをresumeせずfreshなSol GeneratorでStep 2へ戻す。
- **不合格（spec-issue）** → feedback が「仕様自体の欠陥」と分類した場合は Generator に差し戻さない。
  Retry CountとModel Tierを消費せず、Planner に feedback を渡して契約・仕様の修正を依頼し、その後 Step 2 からやり直す。
