...
|---|---|---|---|
| 企画 | `planner` | 短い指示 → 選択式ヒアリング → 詳細仕様。「何を作るか」だけ | `docs/spec.md` |
...
1. **永続ガイダンスを no-overwrite で用意する** — `CLAUDE.md` / `AGENTS.md` が無ければ
   `templates/` から生成する。既にある場合は上書きせず、`docs/harness-guidance.md` に追記候補を残す。
...
3. もしくは **`/harness` コマンド** を案内する — `docs/` 雛形と永続ガイダンスの生成からループ開始までを行う。
