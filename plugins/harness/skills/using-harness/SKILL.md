...
サブエージェントではなく **オーケストレーター（メインエージェント）だけ** が書く。
...
1. **永続ガイダンス、runtime設定、docs雛形を no-overwrite で用意する** — `CLAUDE.md` / `AGENTS.md` が無ければ
   ...
   `.harness/config.json` が無ければ共有設定の雛形を作り、既存設定は編集しない。個人上書きは
   `.harness/config.local.json` に必要な項目だけ書く（`.harness/.gitignore` がこのファイルを除外する）。
