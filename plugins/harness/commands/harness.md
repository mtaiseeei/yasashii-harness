...
1. **docs雛形を用意する**（無ければ作る。既存があれば上書きしない）:
   ...
   - `docs/sprints/sprint-N.md`（スプリント契約。Planner が書く）
   - `docs/progress/`（Generator が `sprint-N.md` を書く）
...
4. **Step 1（企画）から開始**：
   ...
   - 重要判断が固まってから `docs/spec.md`、必要な `docs/spec/*.md`、
     `docs/sprints/current.md`、初回の `docs/sprints/sprint-N.md` を生成する（brainstorm-before-build）。
